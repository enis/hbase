...
public class RpcRetryingCallerWithFallBack {
  ...
  class ReplicaRegionServerCallable extends RegionServerCallable<Result> {
    ...
    public ReplicaRegionServerCallable(int id) {
      super(RpcRetryingCallerWithFallBack.this.connection,
    ...
    public Result call() throws Exception {
      ...
      // We should build the object only once, but we can't right now as the protobufUtils wants
      // the destination server.
      if (id != HRegionInfo.REPLICA_ID_PRIMARY) {
        r.setStale(true);
        LOG.trace("Returning a stale result");
      }
          ...
          while (it.hasNext()) {
            ...
            if (task.isDone()) {
              ...
              if (!task.isCancelled()) {
                try {
                  Result r = task.get();
                  if (r.isStale() && LOG.isTraceEnabled()){
                    LOG.trace("STALE DATA!!");
                  }
                  return r;
                } catch (ExecutionException e) {
                  Throwable t = translateException(e);
