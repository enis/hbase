...
public class TestMetaReaderEditor {
  ...
  public void testMetaReaderGetColumnMethods() {
    ...
    Assert.assertArrayEquals(Bytes.toBytes(HConstants.SERVER_QUALIFIER_STR
      + MetaReader.META_REPLICA_ID_DELIMITER + "00042"), MetaReader.getServerColumn(42));
    ...
    Assert.assertArrayEquals(Bytes.toBytes(HConstants.STARTCODE_QUALIFIER_STR
      + MetaReader.META_REPLICA_ID_DELIMITER + "00042"), MetaReader.getStartCodeColumn(42));
    ...
    Assert.assertArrayEquals(Bytes.toBytes(HConstants.SEQNUM_QUALIFIER_STR
      + MetaReader.META_REPLICA_ID_DELIMITER + "00042"), MetaReader.getSeqNumColumn(42));
  }

  @Test
  public void testMetaLocationsForRegionReplicas() throws IOException {
    Random random = new Random();
    ServerName serverName0 = new ServerName("foo", 60010, random.nextLong());
    ServerName serverName1 = new ServerName("bar", 60010, random.nextLong());
    ServerName serverName100 = new ServerName("baz", 60010, random.nextLong());

    long regionId = System.currentTimeMillis();
    HRegionInfo primary = new HRegionInfo(TableName.valueOf("table_foo"), HConstants.EMPTY_START_ROW,
      HConstants.EMPTY_END_ROW, false, regionId, 0);
    HRegionInfo replica1 = new HRegionInfo(TableName.valueOf("table_foo"), HConstants.EMPTY_START_ROW,
      HConstants.EMPTY_END_ROW, false, regionId, 1);
    HRegionInfo replica100 = new HRegionInfo(TableName.valueOf("table_foo"), HConstants.EMPTY_START_ROW,
      HConstants.EMPTY_END_ROW, false, regionId, 100);
